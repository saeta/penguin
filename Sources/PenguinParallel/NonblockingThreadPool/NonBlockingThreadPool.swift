...
fileprivate final class PerThreadState<Environment: ConcurrencyPlatform> {
  ...
  func steal() -> Task? {
    let r = rng.next()
    var selectedThreadId = Int(fastFit(r, into: UInt32(pool.totalThreadCount)))
    let step = pool.coprimes[Int(fastFit(r, into: UInt32(pool.coprimes.count)))]
    assert(
  ...
  private func findNonEmptyQueueIndex() -> Int? {
    let r = rng.next()
    let increment =
      pool.totalThreadCount == 1 ? 1 : pool.coprimes[Int(fastFit(r, into: UInt32(pool.coprimes.count)))]
    var threadIndex = Int(fastFit(r, into: UInt32(pool.totalThreadCount)))
